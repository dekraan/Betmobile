...
def load_picks_evaluated() -> pd.DataFrame:
    """
    Laad opgeslagen en gesettelde picks.

    Gebruikt de ontdubbelde view (eerste pick per match+selection) als die
    bestaat, zodat een weddenschap maar EEN keer meetelt. Zonder de view
    (migratie nog niet gedraaid) valt hij terug op de ruwe tabel.
    """
    picks_source = "picks_evaluated"
    exists, _ = relation_exists("picks_evaluated_unique_v")
    if exists:
        picks_source = "picks_evaluated_unique_v"
        print("[picks_evaluated] ontdubbelde view gebruikt (unieke bets)")
    else:
        print("[picks_evaluated] WAARSCHUWING: unieke view ontbreekt; ruwe tabel bevat dubbele picks")
    existing_relation_or_fail(picks_source)
